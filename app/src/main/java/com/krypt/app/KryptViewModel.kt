    ...
    fun sendTextMessage(to: String, text: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (contact.publicKey.isEmpty()) {
                NetworkClient.requestPublicKey(to); return@launch
            }
            try {
    ...
    fun sendFile(to: String, uri: Uri) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                // Enforce allowed types: images, PDF, text
                ...
                val fileName = "krypt_${System.currentTimeMillis()}.$ext"
                ...
                if (failCount > 0) Log.e(TAG, "$failCount/${chunks.size} chunks failed")
                ...
                db.messageDao().insertMessage(MessageEntity(
                    ...
                    content = "[sent: $fileName]", contentType = contentType,
        ...
        when (type) {
            ...
            "file_chunk" -> {
                ...
                try {
                    ...
                    chunkMap[chunk.chunkIndex] = CryptoEngine.decryptFileChunk(chunk, myPrivateKey)
                    incomingChunkMeta[key] = chunk
                    if (chunkMap.size == chunk.totalChunks) {
                        val fullFile = (0 until chunk.totalChunks).flatMap { i ->
                            chunkMap[i]?.toList() ?: return@withContext
                        }.toByteArray()
                        ...
                        db.messageDao().insertMessage(MessageEntity(
                            conversationId = from, fromUuid = from,
                            content = "[received: ${chunk.fileName}]", contentType = contentType,
                            filePath = file.absolutePath, isSent = false
                        ))
                        incomingChunks.remove(key); incomingChunkMeta.remove(key)
                        if (_uiState.value.currentConversation != from) {
                            val contact = db.contactDao().getContact(from)
                            withContext(Dispatchers.Main) { showFileNotification(from, chunk.fileName, contact?.nickname ?: "") }
                        }
                    }
                } catch (e: Exception) {
    ...
    fun startCall(remoteUuid: String) {
        ...
        try {
            webRTCManager = WebRTCManager(
                ...
                onCallEnded = {
                    viewModelScope.launch(Dispatchers.Main) { endCall() }
                }
            )
